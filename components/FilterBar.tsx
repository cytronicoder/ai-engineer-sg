...
export const FilterBar = memo(function FilterBar({
    ...
    const handleDayChange = useCallback((day: DayFilter) => onDayChange(day), [onDayChange]);
    const handleInterestToggle = useCallback((interest: InterestKey) => onInterestToggle(interest), [
        onInterestToggle,
    ]);
    const handleSearchChange = useCallback((value: string) => onSearchChange(value), [onSearchChange]);
    const handleSortChange = useCallback((sort: SortMode) => onSortChange(sort), [onSortChange]);
    ...
    const dayButtons = useMemo(
        ...
        [days, dayFilter, handleDayChange],
    );
               ...
               Filters
                ...
                <div className="mt-4 space-y-4">
                    <label className="block">
                        ...
                        <div
                            className="mt-2 flex min-h-12 items-center gap-2 rounded-xl border border-white/10 bg-black/[0.35] px-3">
                    ...
                    <div>
                        ...
                        <div className="mt-2 flex gap-2 overflow-x-auto pb-1">{dayButtons}</div>
                    </div>

                    <div>
                        <p className="label">Interests</p>
                        <p className="mt-1 text-sm text-zinc-400">Pick a few to sort the schedule.</p>
                        <div className="mt-2 flex gap-2 overflow-x-auto pb-1">{interestButtons}</div>
                    </div>
                        ...
                        <div className="mt-2 grid grid-cols-3 gap-2">
                            {[
                                ...
                                <button
                                    ...
                                    onClick={() => handleSortChange(value as SortMode)}
                                >
                                    {label}
                                </button>
                            ))}
