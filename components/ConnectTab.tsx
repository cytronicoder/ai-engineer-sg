...
import {Download, ExternalLink, Link as LinkIcon, Pencil, QrCode, Share2, UserRound,} from "lucide-react";
            ...
            <div className="surface rounded-2xl p-4">
                {scannedView ? (
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <ActionButton icon={UserRound} label="Create my own card" onClick={onCreateOwn}
                                      className="col-span-2"/>
                    </div>
                ) : (
                    <div className="grid grid-cols-2 gap-2">
...
const Avatar = memo(function Avatar({profile}: { profile: ConnectProfile }) {
    return (
        <div
            className="flex h-24 w-24 items-center justify-center rounded-3xl border border-red-300/20 bg-red-500/15 text-2xl font-semibold text-red-100 shadow-glow">
