...
export const BottomNav = memo(function BottomNav({
    ...
    const handleChange = useCallback((tab: AppTab) => onChange(tab), [onChange]);
    return (
        <>
            <nav
                className="fixed inset-x-0 bottom-0 z-30 hidden border-t border-white/10 bg-zinc-950/95 px-4 py-2 backdrop-blur md:block">
            ...
            <nav
                className="fixed inset-x-0 bottom-0 z-30 border-t border-white/10 bg-zinc-950/95 px-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] pt-2 backdrop-blur md:hidden">
...
function Badge({count}: { count: number }) {
    return (
        <span
            className="absolute right-1 top-1 min-w-5 rounded-full bg-white px-1.5 py-0.5 text-center text-[0.65rem] font-bold text-red-600">
      {count}
    </span>
    );
}
